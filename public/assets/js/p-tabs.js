...
window.scrollToAnchorWithOffset = (anchorId, attempt = 1) => {
    ...
    setTimeout(() => {
        setTimeout(() => {
            ...
            const navEl = document.querySelector("nav.pmenu");
            const tbar = document.querySelector(".tbar");

            const headerHeight = header ? header.offsetHeight : 0;
            const navHeight = navEl ? navEl.offsetHeight : 0;
            const tbarHeight = tbar && tbar.offsetHeight > 0 && tbar.offsetParent !== null ? tbar.offsetHeight : 0;

            const tabsContainer = anchorEl.closest(".ptabs, .tabs-content");
            let tabsHeadersHeight = 0;
            if (tabsContainer) {
                const tabsNav = tabsContainer.querySelector(".ptabs__nav, .ptabs__list, ul.tabs");
                if (tabsNav?.offsetParent !== null) {
                    tabsHeadersHeight = tabsNav.offsetHeight;
                }
            }

            const totalOffset = headerHeight + navHeight + tbarHeight + tabsHeadersHeight + 40;
            const rect = anchorEl.getBoundingClientRect();
            const currentScroll = window.pageYOffset || document.documentElement.scrollTop;
            const targetScroll = currentScroll + rect.top - totalOffset;
            ...
            setTimeout(() => {
                ...
                if (finalRect.top > 50 && attempt < 2) {
                    scrollToAnchorWithOffset(anchorId, attempt + 1);
                }
            }, 300);
...
window.openAccordionChainLocal = (target) => {
...
class PTabs {
    ...
    openFromHash() {
        ...
        const accordionItem = target.closest(".paccordion__item");
        if (accordionItem && this.root.contains(accordionItem)) {
            ...
            setTimeout(() => {
                if (window.openAccordionChain) window.openAccordionChain(target);
                window.scrollToAnchorWithOffset(hash);
            }, 400);
            return;
        }
        ...
        if (tab) {
            this.activateTab(tab, { focus: true, scroll: false });
            setTimeout(() => window.scrollToAnchorWithOffset(hash), 200);
            return;
        }
        ...
        setTimeout(() => window.scrollToAnchorWithOffset(hash), 400);
    }
}
...
(function () {
    if (window.ptabsScrollHandler) return;
    window.ptabsScrollHandler = true;

    let ptabsReady = false;
    let scrollTimeout = null;

    const scrollIfNeeded = () => {
        if (!ptabsReady || !window.location.hash) return;

        const hash = window.location.hash.replace("#", "");
        scrollToAnchorWithOffset(hash, 1);
    };

    const waitForPTabs = () => {
        const ptabsInstances = document.querySelectorAll('.ptabs').length;
        if (ptabsInstances > 0) {
            ptabsReady = true;
            scrollIfNeeded();
            return;
        }
        setTimeout(waitForPTabs, 100);
    };
...
window.openAccordionChain = (target) => {
    ...
    let current = target?.closest(".paccordion__item");
    while (current) {
        window.AccordionInstance.openItemImmediate(current, { setCurrent: current === target.closest(".paccordion__item") });
        current = current.parentElement?.closest(".paccordion__item");
    }
};
